...
function renderEntries(people) {
  ...
  if (people.length === 0) {
    list.innerHTML = "<p>No entries match.</p>";
    return;
  }
...
function filterEntriesBySearch() {
  const input = document.getElementById("entriesSearchInput");
  if (!input) return;
  ...
  const filtered = window.allEntries.filter(p =>
...
async function saveEntry(id, card) {
  ...
  try {
    ...
    if (data.success) {
      ...
      const idx = window.allEntries.findIndex(p => p.entryNumber === id);
      if (idx !== -1) {
        window.allEntries[idx].name = name;
        window.allEntries[idx].info = info;
      }
    } else {
...
async function deleteEntry(id, card) {
  ...
  try {
    ...
    if (data.success) {
      card.remove();
      window.allEntries = window.allEntries.filter(p => p.entryNumber !== id);
    } else {
...
/* ⭐ CREATE ENTRY: duplicate-name check + clear form after submit */

function namesLikelyMatch(nameA, nameB) {
  const wordsA = nameA.trim().toLowerCase().split(/\s+/).filter(Boolean);
  const wordsB = nameB.trim().toLowerCase().split(/\s+/).filter(Boolean);
  return wordsA.some(word => wordsB.includes(word));
}

function getCreateEntryFields() {
  return {
...
function clearCreateEntryFields() {
  ["newName", "info1", "info2", "info3"].forEach(id => {
...
async function handleCreateEntry() {
  const { name, info } = getCreateEntryFields();
  ...
  try {
    const res = await fetch("/admin/entries");
    const people = await res.json();
    const matches = people.filter(p => namesLikelyMatch(name, p.name || ""));

    if (matches.length > 0) {
      showDuplicateWarning(name, info, matches);
    } else {
      await submitNewEntry(name, info);
    }
  } catch (err) {
    console.error("Error checking for duplicate names:", err);
    // If the check itself fails, don't block entry creation — just create it.
    await submitNewEntry(name, info);
  }
}

function showDuplicateWarning(name, info, matches) {
  const box = document.getElementById("duplicateWarning");
  if (!box) {
    submitNewEntry(name, info);
    return;
  }

  const matchList = matches.map(m => `
    <div class="duplicate-match">
      <strong>#${m.entryNumber} ${escapeAttr(m.name || "")}</strong>
      <p>${escapeAttr(m.info[0] || "")}</p>
      <p>${escapeAttr(m.info[1] || "")}</p>
      <p>${escapeAttr(m.info[2] || "")}</p>
    </div>
  `).join("");
  ...
  box.innerHTML = `
    <p>You already have ${plural ? "entries" : "an entry"} that ${plural ? "look" : "looks"} like it could match "${escapeAttr(name)}":</p>
    ${matchList}
    <p>Are you sure "${escapeAttr(name)}" is a new entry?</p>
    <button id="confirmCreateAnyway" class="primary-button small">Yes, it's new</button>
    <button id="cancelCreate" class="secondary-button small">Cancel</button>
  `;
  box.style.display = "block";

  document.getElementById("confirmCreateAnyway").addEventListener("click", async () => {
...
async function submitNewEntry(name, info) {
  try {
    ...
    const data = await res.json();
    if (data.success) {
      alert(`Entry #${data.entryNumber} created.`);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (createBtn) {
    createBtn.addEventListener("click", handleCreateEntry);
  }
  ...
  if (entriesSearchInput) {
    entriesSearchInput.addEventListener("input", filterEntriesBySearch);
  }
